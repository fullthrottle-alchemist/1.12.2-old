...
public class EntityColdPig extends EntityColdAnimal {
	...
	public EntityColdPig(final World world) {
		super(world);
	...
	protected void initEntityAI() {
		...
		this.tasks.addTask(3, new EntityAITempt(this, 1.2D, Items.CARROT_ON_A_STICK, false));
	...
	public static void registerFixesPig(final DataFixer fixer) {
		EntityLiving.registerFixesMob(fixer, EntityColdPig.class);
	}
	...
	public void travel(final float strafe, final float vertical, final float forward) {
		final Entity entity = this.getPassengers().isEmpty() ? null : (Entity) this.getPassengers().get(0);
	...
	@Override
	public EntityColdPig createChild(final EntityAgeable ageable) {
		return new EntityColdPig(this.world);
	}
